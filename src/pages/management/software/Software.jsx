            ...
            <Box sx={{ width: '100%' }}>
                ...
                <Grid container spacing={3} sx={{ mb: 3 }}>
                    ...
                    <Grid item xs={12} sm={12} md={5} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <Button component={Link} to='/management/software/add-software'
                            variant="contained"
                            color="primary"
                            startIcon={<AddIcon />}
                            sx={{
                                ...
                                <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                    <TableCell component="th" scope="row">
                                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                            <Box>
                                                <Typography sx={{ fontSize: '15px', color: 'primary.main', fontWeight: 500 }}>{software.name}</Typography>
                                                <Typography sx={{ fontSize: '15px', color: '#666' }}>{software.type}</Typography>
                                            </Box>
                                        </Box>
                                    </TableCell>

                                    <TableCell>{software.description}</TableCell>
                                    <TableCell>{new Date(software.licenseExpire).toLocaleDateString()}</TableCell>
                                    <TableCell>
                                        <RenderStatus isActive={software.status} />
                                    </TableCell>
                                    <TableCell align='right'>
                                        <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                                            <IconButton onClick={(e) => handleMenuOpen(e, software.id)}>
                                                <MoreVertIcon />
                                            </IconButton>
                                            <Menu
                                                anchorEl={anchorEl}
                                                keepMounted
                                                open={Boolean(anchorEl) && menuSoftwareId === software.id}
                                                onClose={handleMenuClose}
                                            >
                                                <MenuItem component={Link} to={`/management/software/add-software/${software.id}`} onClick={handleMenuClose}>
                                                    <EditIcon sx={{ mr: 1 }} /> Edit 
                                                </MenuItem>
                                                {status === true && (
                                                    <MenuItem onClick={() => { setDisableId(software.id); handleMenuClose(); }}>
